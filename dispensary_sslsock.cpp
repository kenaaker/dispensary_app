...
void dispensary_sslsock::connect_to_db() {
...
void dispensary_sslsock::incomingConnection(qintptr socketDescriptor) {
...
void dispensary_sslsock::send_line(QString &a_line) {
    socket->write(a_line.toLatin1(), a_line.size());
}

QString dispensary_sslsock::recv_line() {
    QByteArray in_buff(256, ' ');
    socket->readLine(in_buff.size());
    return QString(in_buff);
}

void dispensary_sslsock::startServerEncryption() {
    bool b;
...
void dispensary_sslsock::connectSocketSignals() {
...
void dispensary_sslsock::slot_encrypted() {
    qDebug("dispensary_sslsock::slot_encrypted");
}

void dispensary_sslsock::slot_encryptedBytesWritten(qint64 written) {
    qDebug("dispensary_sslsock::slot_encryptedBytesWritten(%ld)", (long) written);
}

void dispensary_sslsock::slot_modeChanged(QSslSocket::SslMode mode) {
    qDebug("dispensary_sslsock::slot_modeChanged(%d)", mode);
}

void dispensary_sslsock::slot_peerVerifyError(const QSslError &) {
    qDebug("dispensary_sslsock::slot_peerVerifyError");
}

void dispensary_sslsock::slot_sslErrors(const QList<QSslError> &) {
    qDebug("dispensary_sslsock::slot_sslErrors");
}

void dispensary_sslsock::slot_readyRead() {
...
void dispensary_sslsock::slot_connected() {
    qDebug("dispensary_sslsock::slot_connected");
}

void dispensary_sslsock::slot_disconnected() {
    qDebug("dispensary_sslsock::slot_disconnected");
}

void dispensary_sslsock::slot_error(QAbstractSocket::SocketError err) {
    qDebug() << "dispensary_sslsock::slot_error(" << err << ")";
    qDebug() << socket->errorString();
}

void dispensary_sslsock::slot_hostFound() {
    qDebug("dispensary_sslsock::slot_hostFound");
}

void dispensary_sslsock::slot_proxyAuthenticationRequired(const QNetworkProxy &, QAuthenticator *) {
    qDebug("dispensary_sslsock::slot_proxyAuthenticationRequired");
}

void dispensary_sslsock::slot_stateChanged(QAbstractSocket::SocketState state) {
